...
function RenderMenuItem({ subject ,deleteSubject, editSubject}) {
    return(
        <tr className="gradeA" role="row" key={subject._id}>
            <td>{subject._id}.</td>
            <td>{subject.name}</td>
            <td>{moment(subject.updatedAt).format("YYYY-MM-DD HH:mm:ss")}</td>
            <td>{(subject.iastatus == 1) ? <div className="badge badge-success">Active</div> : 
            <div className="badge badge-danger">Inactive</div>}</td>
            <td>
                <CardImg height="100px" width="10px" src={baseUrl+'public/subjects/'+subject.image} alt={subject.name}></CardImg>
            </td>
            <td>
                <Edit subject={subject} editSubject={editSubject}></Edit>
                <Delete subjectid={subject._id} deleteSubject={deleteSubject}></Delete>
...
function SubjectPage (props) {
    ...
    else if (props.subject !== undefined ) {
        const listdata = props.subject.map((subject, key) => {
            return (
                <RenderMenuItem subject={subject} key={key} deleteSubject={props.deleteSubject} editSubject={props.editSubject} ></RenderMenuItem>
            )
        })
        return (
            ...
            <Modal className="modal-lg" isOpen={this.state.isModalOpen} toggle={this.addtoggleModal}>
                    ...
                    <Form onSubmit={onSubmit} 
                        initialValues={{
                            ...
                            position: this.props.subject.position,
                            status: this.props.subject.iastatus
                          }}
